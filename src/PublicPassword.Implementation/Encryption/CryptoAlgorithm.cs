...
namespace PublicPassword.Implementation.Encryption;
...
public class CryptoAlgorithm
{
    ...
    {
        using var alg = CreateAes(password);
    ...
    public static async Task<string> Decrypt(string password, byte[] encryptedData)
    {
        ...
        {
            throw new Exception("Error happened during decryption of data. " +
                                "One of reason is you are trying to decrypt using wrong password.", ex);
        }
    }

    private static SymmetricAlgorithm CreateAes(string password)
    {
        var alg = Aes.Create();
        try
        {
